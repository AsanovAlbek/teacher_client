...
class CollaboratorsBloc extends Bloc<CollaboratorsEvent, CollaboratorsState> {
  ...
  CollaboratorsBloc(this.collaboratorsRepository) : super(const CollaboratorsState.loading()) {
  ...
  FutureOr<void> _fetchCollaborators(FetchCollaboratorsEvent event, Emitter<CollaboratorsState> emit) async {
    try {
      final collaborators = await collaboratorsRepository.courseCollaborators(event.courseId);
  ...
  FutureOr<void> _addCollaboratorByEmail(AddCollaboratorByEmailEvent event, Emitter<CollaboratorsState> emit) async {
    try {
      final teacher = await collaboratorsRepository.addCollaboratorByEmail(event.courseId, event.email);
  ...
  FutureOr<void> _removeCollaboratorById(RemoveCollaboratorByIdEvent event, Emitter<CollaboratorsState> emit) async {
    try {
      final teacher = await collaboratorsRepository.removeCollaboratorById(event.courseId, event.collaboratorId);
      event.onSuccess?.call(teacher);
    } on Exception catch(e,s) {
